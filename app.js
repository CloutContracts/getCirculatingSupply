...
app.get('/getCirculatingSupply', async (req, res) => {
    try {
        var address = '0x0d81d9e21bd7c5bb095535624dcb0759e64b3899';
        if(address == "" || web3.utils.isAddress(address) == false){
            res.send("Invalid Address")
        }else{
            var deployerBalance = await Contract.methods.balanceOf(address).call();
            console.log(deployerBalance, 'balance');

            var totalSupply = await Contract.methods.totalSupply().call();
            console.log(totalSupply, 'supp')
            var circulatingSupply = await totalSupply - deployerBalance;
